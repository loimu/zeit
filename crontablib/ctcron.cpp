...
QString CTCron::exportCron() const {
	...
	exportCron += QLatin1String( "\n" );
    QString exportInfo = tr("File generated by Crontablib the %1.", "Generation Message + current date").arg(QDateTime::currentDateTime().toString(QLocale().dateTimeFormat()));
...
CTSaveStatus CTCron::prepareSaveStatusError(const CommandLineStatus& commandLineStatus) {
	...
	if (commandLineStatus.exitCode == 127)
        detailError = tr("<p><strong>Command:</strong> %1</p><strong>Command could not be started</strong>").arg(commandLineStatus.commandLine);
	else
        detailError = tr("<p><strong>Command:</strong> %1</p><strong>Standard Output :</strong><pre>%2</pre><strong>Error Output :</strong><pre>%3</pre>").arg(commandLineStatus.commandLine).arg(standardOutput).arg(standardError);

    return CTSaveStatus(tr("An error occurred while updating crontab."), detailError);
}
...
CTSaveStatus CTCron::save() {
	...
	if (saveStatus == false) {
        return CTSaveStatus(tr("Unable to open crontab file for writing"), tr("The file %1 could not be opened.").arg(d->tmpFileName));
	}
